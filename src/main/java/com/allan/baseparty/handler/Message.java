...
public final class Message implements Delayed{
    ...
    private volatile boolean mIsRecylcing = false;

    private static final LinkedList<Message> sPool = new LinkedList<>();
    ...
    void recycleUnchecked() {
        ...
        String log = hashCode() + " is recylcing...";
        ...
        synchronized (sPoolSync) {
            if (sPool.size() < MAX_POOL_SIZE) {
                if(DEBUG) System.out.println(log + "add into Pool");
                sPool.addLast(this);
            }
        }
    }
    ...
    public static Message obtain() {
        synchronized (sPoolSync) {
            if (!sPool.isEmpty() && sPool.size() > CORE_POOL_SIZE) {
                Message m = sPool.removeFirst();
                m.mIsRecylcing = false;
                return m;
            }
        }
        return new Message();
