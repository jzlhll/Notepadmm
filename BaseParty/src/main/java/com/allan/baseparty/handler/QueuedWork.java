...
public final class QueuedWork {
    ...
    private static Handler getHandler() {
        ...
        synchronized (sLock) {
            if (sHandler == null) {
                HandlerThread handlerThread = new HandlerThread("queued_work_thread");
                handlerThread.start();
    ...
    public static void waitToFinish() {
        ...
        synchronized (sLock) {
            if (handler.removeMessages(QueuedWorkHandler.MSG_RUN)) {
    ...
    public static void queue(Runnable work, boolean shouldDelay) {
        ...
        synchronized (sLock) {
            ...
            if (shouldDelay && sCanDelay) {
                handler.sendMessageDelayed(handler.obtainMessage(QueuedWorkHandler.MSG_RUN), DELAY);
            } else {
                handler.sendMessage(handler.obtainMessage(QueuedWorkHandler.MSG_RUN));
            }
        }
    }
