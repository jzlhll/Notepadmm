...
| `src/main/java/com/allan/atools/tools/modulenotepad/manager/MarkdownCodeBlockManager.java` | 代码块整块圆角矩形背景：后台解析收集代码块行区间，段落样式类 `md-code-block-first/mid/last/single` 在 TextFlow 上画背景；左缘对齐正文起始线（内缩 65px，换行 105px）、右缘贴 TextFlow 右缘（`.editor-area` 右 padding 已收窄 VirtualFlow），12px 圆角；内容再内缩 10px（复合选择器覆盖 `.paragraph-text` 的 padding） |
